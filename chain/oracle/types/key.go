...
// GetPythProPriceStoreKey returns the full store key for a PythPro price state.
// feedID is encoded as 4 bytes big-endian, which preserves lexicographic ordering
// for correct prefix-store iteration.
func GetPythProPriceStoreKey(feedID uint32) []byte {
	key := make([]byte, 5)
	key[0] = PythProPriceKey[0]
	binary.BigEndian.PutUint32(key[1:], feedID)
	return key
}

// GetPythProFeedIDFromIterKey decodes the feedID from a prefix-store iterator key suffix (4 bytes big-endian).
func GetPythProFeedIDFromIterKey(iterKey []byte) uint32 {
	if len(iterKey) == 4 {
		return binary.BigEndian.Uint32(iterKey)
	}
	return 0
}

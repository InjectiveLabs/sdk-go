...
type CrossMarginParams struct {
	...
	// max_active_derivative_markets_per_pool defines the maximum number of
	// active derivative markets (positions or orders) a cross-margin
	...
	// emergency_paused defines whether cross margin is in emergency pause
	// mode. When true:
	// - User-submitted cross-margin orders (derivative + spot, limit / market
	//   / conditional, reduce-only included) are rejected at validation.
	// - Paused resting makers are inert: every matcher and TOB scan skips
	//   them uniformly, including against system-generated liquidation
	//   taker orders.
	// - Liquidation of paused pools is still permitted; system taker orders
	//   close positions against non-paused liquidity or defer to settlement.
	// - Collateral-decreasing operations (withdrawals, transfers, margin
	//   decrease) remain subject to the marker-aware admission floor with
	//   positive UPnL stripped first, NOT to a flat isolated-margin
	//   maintenance level.
	...
	// backstop_margin_ratio defines the target health buffer above
	// maintenance margin for graduated liquidation. Liquidation stops when
	// EquityLiquidation >= MM * (1 + ratio).
	...
	// partial_liquidation_ratio is the fraction of the shortfall to target
	// per liquidation tx. 1.0 = close enough to fully restore health; <1.0
	// = close only a fraction per tx.
	...
	// liquidation_cooldown_blocks is the minimum number of blocks between
	// successive partial liquidations on the same cross-margin pool.
	...
	// max_cross_margin_spot_orders_per_subaccount_per_denom caps the number of
	// open spot orders (resting limit + transient limit + transient market)
	// that a cross-margin-mode subaccount may hold for any single locking
	// denom. The cap protocol-bounds spot cancel-first work performed during
	// cross-margin pool liquidation so the validator-work amplifier under fixed
	// gas is bounded by a numeric protocol limit rather than only by fixed
	// tx-level pricing. Enforcement
...
var fileDescriptor_0b5851fb01a33564 = []byte{
	...
	0x1f, 0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0xff, 0xd4, 0x5a, 0x4b, 0x6c, 0x24, 0x49,
	0x5a, 0xee, 0xf4, 0xb3, 0xfc, 0xdb, 0x65, 0x97, 0xc3, 0xaf, 0x72, 0x3f, 0x6c, 0x77, 0x76, 0xf7,
	0xb4, 0xa7, 0x67, 0xc7, 0xde, 0xe9, 0x51, 0xaf, 0x86, 0x1e, 0x5e, 0xb6, 0xcb, 0x9e, 0xae, 0x99,
	...
	,
}

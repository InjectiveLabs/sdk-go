...
type Params struct {
	...
	// swap_params groups the native spot swap configuration.
	SwapParams SwapParams `protobuf:"bytes,40,opt,name=swap_params,json=swapParams,proto3" json:"swap_params"`
}
...
func (m *Params) GetSwapParams() SwapParams {
	if m != nil {
		return m.SwapParams
	}
	return SwapParams{}
...
type SwapParams struct {
	// enabled is the global switch for the native spot swap execution path.
	Enabled bool `protobuf:"varint,1,opt,name=enabled,proto3" json:"enabled,omitempty"`
	// allowed_markets is the strict allowlist of spot market IDs that swap
	// routes may traverse. An empty list means no markets are swappable.
	AllowedMarkets []string `protobuf:"bytes,2,rep,name=allowed_markets,json=allowedMarkets,proto3" json:"allowed_markets,omitempty"`
}

func (m *SwapParams) Reset()         { *m = SwapParams{} }
func (m *SwapParams) String() string { return proto.CompactTextString(m) }
func (*SwapParams) ProtoMessage()    {}
func (*SwapParams) Descriptor() ([]byte, []int) {
	return fileDescriptor_0b5851fb01a33564, []int{2}
}
func (m *SwapParams) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *SwapParams) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_SwapParams.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *SwapParams) XXX_Merge(src proto.Message) {
	xxx_messageInfo_SwapParams.Merge(m, src)
}
func (m *SwapParams) XXX_Size() int {
	return m.Size()
}
func (m *SwapParams) XXX_DiscardUnknown() {
	xxx_messageInfo_SwapParams.DiscardUnknown(m)
}

var xxx_messageInfo_SwapParams proto.InternalMessageInfo

func (m *SwapParams) GetEnabled() bool {
	if m != nil {
		return m.Enabled
	}
	return false
...
func (m *SwapParams) GetAllowedMarkets() []string {
	if m != nil {
		return m.AllowedMarkets
	}
	return nil
}

type CrossMarginParams struct {
	// positive_upnl_haircut_rate defines the haircut applied to positive
	// unrealized PnL for cross-margin admission checks.
	...
	// fees_buffer defines a fixed notional buffer (quote-denom units)
	// subtracted from cross-margin equity calculations.
	...
	// enabled_quote_denoms defines the quote denoms that are eligible for
	// cross-margin pools.
	...
	// perpetual_enabled defines whether perpetual derivative markets are
	// eligible for cross margin.
	...
	// expiry_enabled defines whether expiry futures derivative markets are
	// eligible for cross margin.
	ExpiryEnabled bool `protobuf:"varint,5,opt,name=expiry_enabled,json=expiryEnabled,proto3" json:"expiry_enabled,omitempty"`
	// max_active_derivative_markets_per_pool is an optional additional admission
	// cap when a cross-margin subaccount introduces a new derivative carrier in a
	// quote-denom pool. Lowering it does not invalidate grandfathered carriers or
	// prohibit activity in an already-counted market.
	...
	// emergency_paused defines whether new cross-margin activation and
	// risk-increasing activity are paused. Risk reduction, order cancellation,
	// authenticated liquidation, and scheduled settlement remain available.
	...
	// backstop_margin_ratio sets the pool-health target of a liquidation. The
	// configured RFQ router may close one bounded slice of one position per
	// transaction (a one-pair partial) and continue slice by slice from bare
	// maintenance toward this target while the liquidation cooldown window is
	// active; a canonical full-pool close remains available.
	BackstopMarginRatio cosmossdk_io_math.LegacyDec `protobuf:"bytes,8,opt,name=backstop_margin_ratio,json=backstopMarginRatio,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"backstop_margin_ratio"`
	// partial_liquidation_ratio is the fraction of the distance to the backstop
	// target that one liquidation transaction may close. For a configured-RFQ
	// one-pair partial it is a fresh per-transaction budget applied to current
	// state, not a per-window limit: sequential slices may cumulatively exceed
	// the first slice's budget.
	...
	// liquidation_cooldown_blocks is the rolling window, refreshed by every
	// successful liquidation slice, during which a configured RFQ router may
	// continue one-pair partial slices against a pool that is at or above bare
	// maintenance but below the backstop target. A value of 0 or 1 disables
	// that continuation; the public orderbook path keeps rejecting that band.
	LiquidationCooldownBlocks uint64 `protobuf:"varint,10,opt,name=liquidation_cooldown_blocks,json=liquidationCooldownBlocks,proto3" json:"liquidation_cooldown_blocks,omitempty"`
	// max_cross_margin_spot_orders_per_subaccount_per_denom is an additional,
	// possibly tighter, spot-order admission cap for each cross-margin
	// subaccount and locking denom.
	...
	// liquidation_rfq_contract_address is the bounded set of RFQ router contracts
	...
	// be empty.
	LiquidationRfqContractAddress []string `protobuf:"bytes,15,rep,name=liquidation_rfq_contract_address,json=liquidationRfqContractAddress,proto3" json:"liquidation_rfq_contract_address,omitempty"`
	// max_isolated_active_derivative_markets_per_subaccount_per_denom bounds how
	// many isolated-mode derivative markets a subaccount with cross-margin
	// exposure may be active in per quote denom. Pool walks skip isolated-mode
	// markets, so this cap bounds the walk work an account can force on pool
	// builds and liquidation sweeps. Zero-cost for accounts without cross
	// exposure.
	MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom uint32 `protobuf:"varint,16,opt,name=max_isolated_active_derivative_markets_per_subaccount_per_denom,json=maxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom,proto3" json:"max_isolated_active_derivative_markets_per_subaccount_per_denom,omitempty"`
}
...
func (m *CrossMarginParams) GetLiquidationCooldownBlocks() uint64 {
	if m != nil {
		return m.LiquidationCooldownBlocks
	}
	return 0
}

func (m *CrossMarginParams) GetMaxCrossMarginSpotOrdersPerSubaccountPerDenom() uint32 {
	if m != nil {
...
func (m *CrossMarginParams) GetLiquidationRfqContractAddress() []string {
	if m != nil {
		return m.LiquidationRfqContractAddress
	}
	return nil
}

func (m *CrossMarginParams) GetMaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom() uint32 {
	if m != nil {
		return m.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom
	}
	return 0
}
...
type TradeLog struct {
	...
	// readable format)
	Notional cosmossdk_io_math.LegacyDec `protobuf:"bytes,8,opt,name=notional,proto3,customtype=cosmossdk.io/math.LegacyDec" json:"notional"`
}
...
func (*SubaccountRiskProfile) Descriptor() ([]byte, []int) {
	return fileDescriptor_0b5851fb01a33564, []int{39}
}
...
func init() {
	...
	proto.RegisterType((*SwapParams)(nil), "injective.exchange.v2.SwapParams")
...
var fileDescriptor_0b5851fb01a33564 = []byte{
	...
	,
}
...
func (this *Params) Equal(that interface{}) bool {
	...
	if !this.SwapParams.Equal(&that1.SwapParams) {
		return false
	}
	return true
}
func (this *SwapParams) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*SwapParams)
	if !ok {
		that2, ok := that.(SwapParams)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if len(this.AllowedMarkets) != len(that1.AllowedMarkets) {
		return false
	}
	for i := range this.AllowedMarkets {
		if this.AllowedMarkets[i] != that1.AllowedMarkets[i] {
			return false
		}
	}
	return true
...
func (this *CrossMarginParams) Equal(that interface{}) bool {
	...
	if !this.BackstopMarginRatio.Equal(that1.BackstopMarginRatio) {
		return false
	}
	if !this.PartialLiquidationRatio.Equal(that1.PartialLiquidationRatio) {
		return false
	}
	if this.LiquidationCooldownBlocks != that1.LiquidationCooldownBlocks {
		return false
	}
	...
	if len(this.LiquidationRfqContractAddress) != len(that1.LiquidationRfqContractAddress) {
		return false
	}
	for i := range this.LiquidationRfqContractAddress {
		if this.LiquidationRfqContractAddress[i] != that1.LiquidationRfqContractAddress[i] {
			return false
		}
	}
	if this.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom != that1.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom {
		return false
	}
	return true
}
...
func (m *SwapParams) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *SwapParams) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *SwapParams) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if len(m.AllowedMarkets) > 0 {
		for iNdEx := len(m.AllowedMarkets) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.AllowedMarkets[iNdEx])
			copy(dAtA[i:], m.AllowedMarkets[iNdEx])
			i = encodeVarintExchange(dAtA, i, uint64(len(m.AllowedMarkets[iNdEx])))
			i--
			dAtA[i] = 0x12
		}
	}
...
func (m *CrossMarginParams) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if m.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom != 0 {
		i = encodeVarintExchange(dAtA, i, uint64(m.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0x80
	}
	if len(m.LiquidationRfqContractAddress) > 0 {
		for iNdEx := len(m.LiquidationRfqContractAddress) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.LiquidationRfqContractAddress[iNdEx])
			copy(dAtA[i:], m.LiquidationRfqContractAddress[iNdEx])
			i = encodeVarintExchange(dAtA, i, uint64(len(m.LiquidationRfqContractAddress[iNdEx])))
			i--
			dAtA[i] = 0x7a
		}
	}
	...
	if m.LiquidationCooldownBlocks != 0 {
		i = encodeVarintExchange(dAtA, i, uint64(m.LiquidationCooldownBlocks))
		i--
		dAtA[i] = 0x50
	}
	{
	...
	dAtA[i] = 0x4a
	{
		size := m.BackstopMarginRatio.Size()
		i -= size
		if _, err := m.BackstopMarginRatio.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintExchange(dAtA, i, uint64(size))
	}
...
func (m *SwapParams) Size() (n int) {
	...
	if len(m.AllowedMarkets) > 0 {
		for _, s := range m.AllowedMarkets {
			l = len(s)
			n += 1 + l + sovExchange(uint64(l))
		}
	}
...
func (m *CrossMarginParams) Size() (n int) {
	...
	l = m.BackstopMarginRatio.Size()
	n += 1 + l + sovExchange(uint64(l))
	l = m.PartialLiquidationRatio.Size()
	n += 1 + l + sovExchange(uint64(l))
	if m.LiquidationCooldownBlocks != 0 {
		n += 1 + sovExchange(uint64(m.LiquidationCooldownBlocks))
	}
	if m.MaxCrossMarginSpotOrdersPerSubaccountPerDenom != 0 {
	...
	n += 1 + l + sovExchange(uint64(l))
	if len(m.LiquidationRfqContractAddress) > 0 {
		for _, s := range m.LiquidationRfqContractAddress {
			l = len(s)
			n += 1 + l + sovExchange(uint64(l))
		}
	}
	if m.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom != 0 {
		n += 2 + sovExchange(uint64(m.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom))
	}
	return n
}
...
func (m *SwapParams) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: SwapParams: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SwapParams: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field AllowedMarkets", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.AllowedMarkets = append(m.AllowedMarkets, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipExchange(dAtA[iNdEx:])
...
func (m *CrossMarginParams) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 8:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field BackstopMarginRatio", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.BackstopMarginRatio.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 9:
		...
		case 10:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field LiquidationCooldownBlocks", wireType)
			}
			m.LiquidationCooldownBlocks = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowExchange
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.LiquidationCooldownBlocks |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 13:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxCrossMarginSpotOrdersPerSubaccountPerDenom", wireType)
			}
			m.MaxCrossMarginSpotOrdersPerSubaccountPerDenom = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowExchange
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.MaxCrossMarginSpotOrdersPerSubaccountPerDenom |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 15:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field LiquidationRfqContractAddress", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.LiquidationRfqContractAddress = append(m.LiquidationRfqContractAddress, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		case 16:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom", wireType)
			}
			m.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowExchange
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.MaxIsolatedActiveDerivativeMarketsPerSubaccountPerDenom |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}

...
// a live spot limit order. isTransient must match the order's physical store
// class because resting and transient buys use different fee rates.
// GetUnfilledMarginHoldAndMarginDenom deliberately performs notional and fee
// operations in the runtime order; callers must not replace this with an
// algebraically collapsed product.

...
import (
	...
	_ "github.com/InjectiveLabs/injective-core/injective-chain/crypto/codec"
...
//	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, chaintypes.BIP44HDPath, EthSecp256k1)
//	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	...
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, chaintypes.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := hdwallet.MustParseDerivationPath(chaintypes.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)

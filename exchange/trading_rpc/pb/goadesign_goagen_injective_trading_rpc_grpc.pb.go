...
func (c *injectiveTradingRPCClient) GetTradingStats(ctx context.Context, in *GetTradingStatsRequest, opts ...grpc.CallOption) (*GetTradingStatsResponse, error) {
	out := new(GetTradingStatsResponse)
	err := c.cc.Invoke(ctx, "/injective_trading_rpc.InjectiveTradingRPC/GetTradingStats", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _InjectiveTradingRPC_GetTradingStats_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetTradingStatsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(InjectiveTradingRPCServer).GetTradingStats(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/injective_trading_rpc.InjectiveTradingRPC/GetTradingStats",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(InjectiveTradingRPCServer).GetTradingStats(ctx, req.(*GetTradingStatsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
